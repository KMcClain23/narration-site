...
export async function POST(req: Request) {
  try {
    ...
    const { title, author = "", cover_url = "", status = "audition", deadline, notes = "", author_notes = "", links = [], co_narrator = "", sort_order = 0, chapters = [], subtitle = "", tags = [], description = "", audible_link = "", ar_link = "", word_count = 0, first15_due, pfh_rate = 0 } = body;
    if (!title?.trim()) return NextResponse.json({ error: "Title required." }, { status: 400 });
    const { data, error } = await supabaseAdmin
      .from("board_cards")
      .insert({ title: title.trim(), author, cover_url, status, deadline: deadline || null, notes, author_notes, links, co_narrator, sort_order, chapters, subtitle, tags, description, audible_link, ar_link, word_count, first15_due: first15_due || null, pfh_rate })
      .select().single();
...
export async function PUT(req: Request) {
  try {
    ...
    // Admin full update
    const allowed = ["title", "author", "cover_url", "status", "deadline", "notes", "author_notes", "links", "co_narrator", "sort_order", "chapters", "subtitle", "tags", "description", "audible_link", "ar_link", "word_count", "first15_due", "pfh_rate"];

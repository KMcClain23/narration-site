...
export default function BoardPage() {
  ...
  const [importBooks, setImportBooks] = useState<any[]>([]);
          ...
          <div className="w-full max-w-lg bg-[#0A0D3A] border border-[#1A2070] rounded-2xl shadow-2xl my-8">
            <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-white/8">
              <div>
                ...
                <p className="text-xs text-white/40 mt-0.5">Add existing books to the board</p>
              </div>
          ...
          <div className="w-full max-w-2xl bg-[#0A0D3A] border border-[#1A2070] rounded-2xl shadow-2xl my-8">
            ...
            <div className="px-6 py-5 space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <label className="block"><span className="text-[11px] uppercase tracking-[0.18em] text-white/40 font-medium">Subtitle</span><input type="text" value={form.subtitle} onChange={e=>setForm(p=>({...p,subtitle:e.target.value}))} placeholder="e.g. Book 4" className="mt-1.5 w-full rounded-lg bg-black/30 border border-white/8 px-3 py-2.5 text-sm text-white placeholder:text-white/20 focus:outline-none focus:border-[#D4AF37]/40 transition"/></label>
                ...
                <label className="block">
                  ...
                  <div className="mt-1.5 space-y-2">
                    ...
                    <input value={form.cover_url} onChange={e=>setForm(p=>({...p,cover_url:e.target.value}))}
                      ...
                      className="w-full rounded-lg bg-black/30 border border-white/8 px-3 py-2 text-xs text-white/60 focus:outline-none focus:border-[#D4AF37]/40 transition"/>
                  </div>
                </label>
                <label className="block"><span className="text-[11px] uppercase tracking-[0.18em] text-white/40 font-medium">Audible link</span><input type="text" value={form.audible_link} onChange={e=>setForm(p=>({...p,audible_link:e.target.value}))} className="mt-1.5 w-full rounded-lg bg-black/30 border border-white/8 px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#D4AF37]/40 transition"/></label>
                <label className="block"><span className="text-[11px] uppercase tracking-[0.18em] text-white/40 font-medium">Republic link</span><input type="text" value={form.ar_link} onChange={e=>setForm(p=>({...p,ar_link:e.target.value}))} className="mt-1.5 w-full rounded-lg bg-black/30 border border-white/8 px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#D4AF37]/40 transition"/></label>
                <label className="block">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <select value={form.deadline ? form.deadline.split("-")[1] : ""}
                      ...
                      className="flex-1 rounded-lg bg-black/30 border border-white/8 px-2 py-2.5 text-sm text-white appearance-none focus:outline-none focus:border-[#D4AF37]/40">
                    ...
                    <select value={form.deadline ? form.deadline.split("-")[2] : ""}
                      ...
                      className="w-20 rounded-lg bg-black/30 border border-white/8 px-2 py-2.5 text-sm text-white appearance-none focus:outline-none focus:border-[#D4AF37]/40">
                    ...
                    <select value={form.deadline ? form.deadline.split("-")[0] : ""}
                      ...
                      className="w-24 rounded-lg bg-black/30 border border-white/8 px-2 py-2.5 text-sm text-white appearance-none focus:outline-none focus:border-[#D4AF37]/40">
              ...
              <div className="rounded-xl border border-white/6 bg-white/[0.02] p-4 space-y-4">
                <p className="text-[11px] uppercase tracking-[0.2em] text-white/40 font-medium">Production details</p>
                <div className="grid grid-cols-2 gap-4">
                  <label className="block">
                    <span className="text-[11px] uppercase tracking-[0.18em] text-white/40 font-medium">Word count</span>
                    <input type="number" value={form.word_count || ""} onChange={e=>setForm(p=>({...p,word_count:parseInt(e.target.value)||0}))} className="mt-1.5 w-full rounded-lg bg-black/30 border border-white/8 px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#D4AF37]/40 transition"/>
                  </label>
                  <label className="block">
                    <span className="text-[11px] uppercase tracking-[0.18em] text-white/40 font-medium">Payment type</span>
                    <select value={form.payment_type} onChange={e=>setForm(p=>({...p,payment_type:e.target.value}))} className="mt-1.5 w-full rounded-lg bg-black/30 border border-white/8 px-3 py-2.5 text-sm text-white appearance-none focus:outline-none focus:border-[#D4AF37]/40">
                      <option value="pfh">PFH</option>
                      <option value="rs">RS</option>
                      <option value="rs_plus">RS+</option>
                    </select>
                  </label>
                </div>
                {(form.payment_type === "pfh" || form.payment_type === "rs_plus") && (
                  <label className="block">
                    <span className="text-[11px] uppercase tracking-[0.18em] text-white/40 font-medium">{form.payment_type === "rs_plus" ? "RS+ stipend" : "PFH rate"}</span>
                    <input type="number" value={form.pfh_rate || ""} onChange={e=>setForm(p=>({...p,pfh_rate:parseFloat(e.target.value)||0}))} className="mt-1.5 w-full rounded-lg bg-black/30 border border-white/8 px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#D4AF37]/40 transition"/>
                  </label>
                )}
              </div>

              <label className="block">
                <span className="text-[11px] uppercase tracking-[0.18em] text-white/40 font-medium">Description</span>
                <textarea value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))} rows={2}
                  className="mt-1.5 w-full rounded-lg bg-black/30 border border-white/8 px-3 py-2.5 text-sm text-white focus:outline-none focus:border-[#D4AF37]/40 resize-none"/>
              </label>
              ...
              <div className="px-3 pb-3 space-y-3">
                ...
                cards.filter(c=>c.status===column.id).sort(sortCards).map(card=>(
                  <div key={card.id} draggable onDragStart={()=>setDragId(card.id)}
                    className={`rounded-xl bg-[#06082E]/80 border border-white/8 hover:border-white/15 transition-all cursor-grab shadow-md group ${dragId===card.id?"opacity-30":""}`}>
                    ...
                    <Link href={`/board/card/${card.id}`} onClick={e=>e.stopPropagation()}>
                      {card.cover_url ? (
                        ...
                        <div className="h-16 rounded-t-xl bg-[#0A0D3A] flex items-center justify-center border-b border-white/5">
                          <span className="text-xs text-white/20">Open details</span>
                        </div>
                      )}
                    </Link>
                    ...
                    <div className="p-3">
                      ...
                      <Link href={`/board/card/${card.id}`}
                        className="mt-2 inline-flex items-center gap-1 text-[10px] text-[#D4AF37]/60 hover:text-[#D4AF37] font-semibold"
                        onClick={e => e.stopPropagation()}>
                        {card.chapters?.length > 0 ? `${card.chapters.filter((c: any) => c.status === "live").length}/${card.chapters.length} chapters` : "Track chapters"}
                      </Link>
                      ...
                      <div className="mt-3 pt-2.5 border-t border-white/6 flex items-center justify-between gap-2">
                        <select value={card.status}
                          ...
                          className="text-[10px] bg-[#06082E] border border-white/10 rounded-lg px-2 py-1 text-white/50 appearance-none hover:border-white/20 cursor-pointer focus:outline-none">
                          {COLUMNS.map(c=><option key={c.id} value={c.id}>{c.label}</option>)}
                        </select>

                        <div className="flex items-center gap-2 opacity-60 group-hover:opacity-100 transition-opacity">
                          <button type="button" onClick={()=>copyLink(card.author_token)} className="text-white/40 hover:text-[#D4AF37]">
                            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101"/></svg>
                          </button>
                          <button type="button" onClick={()=>startEdit(card)} className="text-white/40 hover:text-white">
                            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5"/></svg>
                          </button>
                          <button type="button" onClick={()=>del(card.id)} className="text-white/40 hover:text-red-400">
                            <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7"/></svg>
                          </button>
                        </div>
                      </div>
                    </div>

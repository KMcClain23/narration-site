...
const FIELDS: { key: keyof Omit<Author, "id" | "name" | "bio">; label: string; placeholder: string }[] = [
  { key: "website", label: "Website", placeholder: "https://co_narratorsite.com" },
...
export default function CoNarratorManager() {
  const [co_narrators, setAuthors] = useState<Author[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      setAuthors(data.co_narrators || []);
    } catch (e: unknown) {
      setError(e instanceof Error ? e.message : "Failed to load co_narrators");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  return (
    <section className="mt-12 pt-12 border-t border-[#1A2070]">
      ...
      ) : co_narrators.length === 0 ? (
        <div className="py-16 text-center rounded-2xl border border-dashed border-[#1A2070]">
          <p className="text-white/20 italic text-sm">No co_narrators yet. Add one above.</p>
        </div>
      ) : (
        <div className="grid gap-3">
          {co_narrators.map((author) => {

...
export function CoNarratorsListClient({ initialRows }: { initialRows: CoNarratorRow[] }) {
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={5} className="px-4 py-8 text-center text-sm text-text-muted">
                  No co-narrators match your search.
                </td>
              </tr>
            ) : (
              filtered.map(row => (
                <tr
                  key={row.id}
                  onClick={() => router.push(`/contacts/co-narrators/${row.slug}`)}
          ...
          <div className="w-full max-w-lg">
            <PersonForm
              ...
              onSaved={(person: Person) => {
                ...
                router.push(`/contacts/co-narrators/${sanitizeName(person.name)}`);
              }}
            />
          </div>
        </div>
